...
require 'spec_helper'
require 'puppet/type/storcli_patrolread'

describe Puppet::Type.type(:storcli_patrolread) do
  describe 'namevar derivation of controller' do
    ...
    it 'derives controller 1 from title /c1' do
      resource = described_class.new(name: '/c1', mode: 'off')
    ...
    it "derives 'all' from title /call" do
      resource = described_class.new(name: '/call', mode: 'auto')
      expect(resource[:controller]).to eq('all')
    end

    it 'allows explicit controller to override title derivation' do
      resource = described_class.new(name: '/c0', controller: 1, mode: 'auto')
      expect(resource[:controller]).to eq(1)
    end
  end
  ...
  describe 'ignore_unsupported parameter' do
    ...
    it 'accepts true' do
      resource = described_class.new(name: '/c0', mode: 'auto', ignore_unsupported: true)
      expect(resource[:ignore_unsupported]).to eq(:true)
    end

    it 'accepts false' do
      resource = described_class.new(name: '/c0', mode: 'auto', ignore_unsupported: false)
      expect(resource[:ignore_unsupported]).to eq(:false)
    end
  end
